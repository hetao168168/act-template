...
var UIPaneLayers = class {
  ...
  doDisplay(renderControls) {
    ...
    if (this.layers.length > 0) {
      new import_obsidian5.ButtonComponent(this.navEl).setIcon("lucide-arrow-left-circle").setClass("clickable-icon").setTooltip(`返回 ${this.layers[this.layers.length - 1].标题}`).onClick(() => this.navInstance.close());
    }
    titleEl.empty();
...
var ComplexAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    containerEl.createEl("p", {
      text: "该高亮块是通过插件的 data.json 文件配置的。为防止无意中更改配置，您需要手动编辑该文件."
    });
    containerEl.createEl("code", { cls: "calloutmanager-edit-callout-appearance-json" }, (el) => {
      el.createEl("pre", { text: complexJson });
    });
    containerEl.createEl("p", {
      text: "您也可以点击下面的按钮两次来重置高亮块。"
    });
    let resetButtonClicked = false;
    const resetButton = new import_obsidian11.ButtonComponent(containerEl).setButtonText("重置高亮块").setClass("calloutmanager-edit-callout-appearance-reset").setWarning().onClick(() => {
      if (!resetButtonClicked) {
        resetButtonClicked = true;
        resetButton.setButtonText("你确定吗?");
        return;
      }
...
var SelectIconPane = class extends UIPane {
  ...
  display() {
    ...
    if (previewLimitOverage > 0) {
      ...
      containerEl.createEl("p", {
        text: `还有 ${pluralIs} ${previewLimitOverage} 更多 ${pluralIcon} 要显示。缩小搜索范围，查看更多内容.`
      });
    }
  }
  /** @override */
...
var PerSchemeAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    new CalloutColorSetting(containerEl, callout).setName("暗色模式").setDesc("为深色方案更改高亮块的颜色.").setColorString(colorDark).onChange((color) => this.setAppearanceOrChangeToUnified({ ...appearance, colorDark: color }));
    new CalloutColorSetting(containerEl, callout).setName(`浅色模式`).setDesc(`为浅色方案更改高亮块的颜色.`).setColorString(colorLight).onChange((color) => this.setAppearanceOrChangeToUnified({ ...appearance, colorLight: color }));
    new CalloutIconSetting(containerEl, callout, plugin, () => nav).setName("图标").setDesc("更高高亮块图标.").setIcon(otherChanges.icon).onChange(
...
var UnifiedAppearanceEditor = class extends AppearanceEditor {
  ...
  render() {
    ...
    new CalloutColorSetting(containerEl, callout).setName("颜色").setDesc("更改高亮块颜色").setColorString(color).onChange((color2) => setAppearance({ type: "unified", otherChanges, color: color2 }));
    new import_obsidian17.Setting(containerEl).setName(`色彩方案`).setDesc(`为 ${其他颜色方案} 配色方案更改高亮块的颜色。`).addButton(
    ...
    new CalloutIconSetting(containerEl, callout, plugin, () => this.nav).setName("图标").setDesc("更高高亮块图标.").setIcon(otherChanges.icon).onChange((icon) => setAppearance({ type: "unified", color, otherChanges: { ...otherChanges, icon } }));
  }
};
...
var MiscEditor = class {
  ...
  createRenameSetting() {
    ...
    desc.createEl("p", { text: "更改该高亮块的名称。" });
    desc.createEl("p", { text: "这不会更新笔记中的任何参考资料！", cls: "mod-warning" });
    let newIdComponent;
...
function renderInfo(app2, callout, containerEl) {
  ...
  contentEl.createEl("h2", { text: "关于此高亮块" });
...
var EditCalloutPanePreview = class {
  ...
  makeEditable() {
    ...
    this.preview.calloutEl.addEventListener("click", () => {
      ...
      contentEl.empty();
      new import_obsidian19.TextAreaComponent(contentEl).setValue(this.previewMarkdown).setPlaceholder("预览 Markdown...").then((c) => {
...
var EditCalloutPane = class extends UIPane {
  ...
  displayControls() {
    ...
    if (!this.viewOnly && callout.sources.length === 1 && callout.sources[0].type === "custom") {
      new import_obsidian20.ButtonComponent(controlsEl).setIcon("lucide-trash").setTooltip("删除高亮块").onClick(() => {
...
var CreateCalloutPane = class extends UIPane {
  constructor(plugin) {
    ...
    this.fieldId = new import_obsidian21.Setting(document.createElement("div")).setHeading().setName("高亮块名称").setDesc("This is how you will refer to your callout in Markdown.").addText((cmp) => {
...
> Contents`,
  ...
  displayControls() {
    const { controlsEl } = this;
    const filter2 = new import_obsidian22.TextComponent(controlsEl).setValue(this.searchQuery).setPlaceholder("过滤器 高亮块...").onChange(this.search.bind(this));
    ...
    if (!this.viewOnly) {
      new import_obsidian22.ButtonComponent(controlsEl).setIcon("lucide-plus").setTooltip("新高亮块").onClick(() => this.nav.open(new CreateCalloutPane(this.plugin))).then(({ buttonEl }) => buttonEl.classList.add("clickable-icon"));
    }
  }
  /** @override */
  restoreState(state) {
...
function createEmptySearchResultDiv() {
  ...
  contentEl.createEl("h2", { text: "未找到高亮块." });
  contentEl.createEl("p", void 0, (el) => {
    el.createSpan({ text: "您的搜索查询 " });
  ...
  contentEl.createDiv({ cls: "calloutmanager-search-error-suggestions" }, (el) => {
    el.createDiv({ text: "尝试搜索:" });
    el.createEl("ul", void 0, (el2) => {
      el2.createEl("li", { text: "按姓名: " }, (el3) => {
        el3.createEl("code", { text: "警告" });
      });
      el2.createEl("li", { text: "按图标: " }, (el3) => {
        el3.createEl("code", { text: "图标：检查" });
      });
      el2.createEl("li", { text: "内置高亮块: " }, (el3) => {
        el3.createEl("code", { text: "来自:obsidian" });
      });
      el2.createEl("li", { text: "主题高亮块: " }, (el3) => {
        el3.createEl("code", { text: "来自：主题" });
      });
      el2.createEl("li", { text: "片段高亮块： " }, (el3) => {
        el3.createEl("code", { text: "来自：我的片段" });
      });
      el2.createEl("li", { text: "自定义高亮块: " }, (el3) => {
        el3.createEl("code", { text: "来自：自定义" });
      });
    });
  });
...
var ManagePluginPane = class extends UIPane {
  ...
  display() {
    ...
    new import_obsidian24.Setting(containerEl).setName("管理高亮块").setDesc("创建或编辑 Markdown 高亮块.").addButton((btn) => {
      btn.setButtonText("管理高亮块");
      btn.onClick(() => this.nav.open(new ManageCalloutsPane(plugin)));
    });
    new import_obsidian24.Setting(containerEl).setHeading().setName("检测高亮块");
    new import_obsidian24.Setting(containerEl).setName("Obsidian").setDesc(
      (() => {
        ...
        container.createDiv({
          text: `查找内置  Obsidian 高亮块${method === "" ? "" : " "}${method}.`
        });
        return desc;
      })()
    ).addToggle((setting) => {
    ...
    new import_obsidian24.Setting(containerEl).setName("主题").setDesc("查找提供主题的高亮块.").addToggle((setting) => {
    ...
    new import_obsidian24.Setting(containerEl).setName("片段").setDesc("在自定义 CSS 片段中查找高亮块。").addToggle((setting) => {
    ...
    new import_obsidian24.Setting(containerEl).setHeading().setName("最新消息").setDesc(`版本 ${this.plugin.manifest.version}`).addExtraButton((btn) => {
    ...
    new import_obsidian24.Setting(containerEl).setName("重置高亮块设置").setDesc("重置您对高亮块所做的所有更改.").addButton(
      withConfirm((btn) => {
    ...
    new import_obsidian24.Setting(containerEl).setName("重置自定义高亮块").setDesc("删除您创建的所有自定义高亮块.").addButton(
      withConfirm((btn) => {
        btn.setButtonText("重置").onClick(() => {
